...
import static dat.enums.RecipeDifficulty.HARD;

public class Populator {

    public static UserDTO[] populateUsersWithRoles() {
...
//            // no need for persisting the recipe, because it is already getting persisted when persisting user. because of the cascade type(PERSIST).
            ...
            UserDTO userDTOjanusRegular = new UserDTO(janus.getUsername(), "user1");
            UserDTO userDTOemilAdmin = new UserDTO(emil.getUsername(), "user1");

            ArrayList<UserDTO> userDTOArrayList = new ArrayList<>();
            userDTOArrayList.add(userDTOjanusRegular);
            userDTOArrayList.add(userDTOemilAdmin);

        return userDTOArrayList.stream().toArray(UserDTO[]::new);
        }
    }
    ...
    @NotNull
    public static Set<Recipes> populateRecipes() {
        ...
        try (EntityManager em = HibernateConfig.getEntityManagerFactory("recipes").createEntityManager()) {
            em.getTransaction().begin();
            recipesSet.forEach(em::persist);
            em.getTransaction().commit();
        }
